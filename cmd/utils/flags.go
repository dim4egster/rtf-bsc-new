...
func SetSecretsConfig(ctx *cli.Context, cfg *secrets.SecretsManagerConfig) {
	...
	if ctx.IsSet(SecretsVaultServerURLFlag.Name) {
		cfg.ServerURL = ctx.String(SecretsVaultServerURLFlag.Name)
	}

	if ctx.IsSet(SecretsVaultTokenFlag.Name) {
		cfg.Token = ctx.String(SecretsVaultTokenFlag.Name)
	}

	if ctx.IsSet(SecretsManagerTypeFlag.Name) {
		cfg.Type = secrets.SecretsManagerType(ctx.String(SecretsManagerTypeFlag.Name))
	}
...
func SetupSecretsManager(ctx *cli.Context, stack *node.Node) {
	...
	case secrets.HashicorpVault:
		if ctx.IsSet(SecretsVaultServerURLFlag.Name) {
			config.ServerURL = ctx.String(SecretsVaultServerURLFlag.Name)
		}
		if ctx.IsSet(SecretsVaultTokenFlag.Name) {
			config.Token = ctx.String(SecretsVaultTokenFlag.Name)
		}
